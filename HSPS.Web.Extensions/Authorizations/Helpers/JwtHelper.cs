...
namespace HSPS.Web.AuthHelper.OverWrite;
...
public class JwtHelper
{
    ...
    public static string IssueJwt(TokenModelJwt tokenModel)
    {
        try
        {
            ...
            var outTime = AppSettings.app(new string[] { "Audience", "OutTime" }).ObjToDouble();//token过期时间
            ...
            var claims = new List<Claim>
                ...
                //过期时间，目前是过期1000秒，可自定义，注意JWT有自己的缓冲过期时间
                new Claim (JwtRegisteredClaimNames.Exp,$"{new DateTimeOffset(DateTime.Now.AddSeconds(outTime)).ToUnixTimeSeconds()}"),
                new Claim(ClaimTypes.Expiration, DateTime.Now.AddSeconds(outTime).ToString()),
            ...
            var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(secret));//长度长于16位
            var creds = new SigningCredentials(key, SecurityAlgorithms.HmacSha256);//HS256加密方式，和登陆加密方式区分
        ...
        {
            Log.Error("颁发JWT出现异常：" + ex);
            Console.WriteLine("颁发JWT出现异常：" + ex);
            return "";
        }
